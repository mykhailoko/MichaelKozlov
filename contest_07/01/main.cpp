...
public:
    void add(IRunnable* client, Time time) override {
        events.emplace_back(client, time);
    ...
    bool next() override {
        if (!events.empty()) {
            Time currentTime;
            std::cout << std::setfill('0') << std::setw(2) << events.front().second.hours << ":"
                      << std::setfill('0') << std::setw(2) << events.front().second.minutes << ":"
                      << std::setfill('0') << std::setw(2) << events.front().second.seconds << " ";
            events.front().first->run();
