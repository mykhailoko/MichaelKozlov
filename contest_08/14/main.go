...
func (b *UserBot) step(board Board) (row, col int, marker string) {
	// Your implementation for UserBot's strategy
	// For example, let's prioritize winning moves and block opponent's winning moves

	// Check for winning moves
	for r := 0; r < 3; r++ {
		for c := 0; c < 3; c++ {
			if board.set(r, c, b.marker) {
				if ok, _ := board.isGameOver(); ok {
					return r, c, b.marker
				}
				board.set(r, c, " ") // Reset the move
			}
		}
	}
	...
	for r := 0; r < 3; r++ {
		for c := 0; c < 3; c++ {
			if board.set(r, c, opponentMarker) {
				if ok, _ := board.isGameOver(); ok {
					return r, c, b.marker
				}
				board.set(r, c, " ") // Reset the move
	...
	// Choose a random empty cell if no winning moves or blocks are found
	for {
...
func (b *UserBot) endGame(winner string) {
	// UserBot doesn't need to learn or adapt after each game
}
